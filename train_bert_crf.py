...
from torch.utils.tensorboard import (
    SummaryWriter,  # from tensorboardX import SummaryWriter
)
...
def main(parser):
    ...
    cwd = Path.cwd()
    data_in = cwd / "data_in"
    train_data_dir = data_in / "privacy-data" / "#@data" / "train"
    ...
    val_data_dir = data_in / "privacy-data" / "#@data" / "valid"
    ...
    print(
        "num of train: {}, num of val: {}".format(train_examples_len, val_examples_len)
    )
    ...
    t_total = (
        len(tr_dl) // model_config.gradient_accumulation_steps * model_config.epochs
    )
    optimizer = AdamW(
        optimizer_grouped_parameters,
    ...
    logger.info(
        "  Gradient Accumulation steps = %d", model_config.gradient_accumulation_steps
    )
    ...
    for _epoch, _ in enumerate(train_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % model_config.gradient_accumulation_steps == 0:
                ...
                # if step % 50 == 0:
                print(
                    "epoch : {}, global_step : {}, tr_loss: {:.3f}, tr_acc: {:.2%}".format(
            ...
            if (
                model_config.logging_steps > 0
                and global_step % model_config.logging_steps == 0
            ):
                if (
                    model_config.evaluate_during_training
                    ...
                    tb_writer.add_scalars(
                        "acc",
                        {"train": tr_acc, "val": eval_summary["eval_acc"]},
                        global_step,
                    )
                    print(
                ...
                print(
                    "Average loss: {} at global step: {}".format(
            ...
            # save model
            if (
                model_config.save_steps > 0
                and global_step % model_config.save_steps == 0
            ):
                eval_summary, list_of_y_real, list_of_pred_tags = evaluate(
                    model, val_dl
                )
                ...
                # Save
                is_best = (
                    eval_summary["eval_acc"] >= best_dev_acc
                ...
                if is_best:
                    ...
                    print(
                        "Saving model checkpoint as best-epoch-{}-step-{}-acc-{:.3f}.bin".format(
                            epoch + 1, global_step, best_dev_acc
                        )
                    )
                    ...
                    print(
                        "Saving model checkpoint as model-epoch-{}-step-{}-acc-{:.3f}.bin".format(
                            epoch + 1, global_step, eval_summary["eval_acc"]
                        )
                    )
    ...
    tb_writer.close()
    print(
        "global_step = {}, average loss = {}".format(global_step, tr_loss / global_step)
    )
...
def save_cr_and_cm(
    ...
    list_of_y_real,
    list_of_pred_tags,
    cr_save_path="classification_report.csv",
...
def plot_confusion_matrix(
    y_true, y_pred, classes, labels, normalize=False, title=None, cmap=plt.cm.Blues
):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_dir", default="data_in", help="Directory containing config.json of data"
    )
    parser.add_argument(
        "--model_dir",
        default="experiments/base_model_with_crf_val",
